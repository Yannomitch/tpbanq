...
<p>Pour créditer le compte du client tapez le montant et valider </p>
            <form action ="index.php?pg=crediter" method ="POST" >
                <fieldset><legend>CREDITER COMPTE</legend>
                <table>
                ...
                <p><input type = "submit" value="Crediter"/> <input type="reset" value="Annuler" /></p>
                </fieldset>
            </form> 
...
if (isset($_POST['numerocompte']) AND isset($_POST['montant']) AND isset($_POST['annee']) AND isset($_POST['mois']) AND isset($_POST['jour']))
{
    ...
    die(print_r($bdd->errorInfo()));//on va récupérer tous les numeros de compte existants
        ...
        else 
        {
            ...
            $req = $bdd->prepare('UPDATE compte SET montcrediter=:montant,solde=:solde,dateupdate=:dateupdate WHERE numerocompte=:numerocompte');
            $req->execute(array(
